...
# // authorize
token=$(curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -u 1560:WHqPESIwLcyTFMtd -d "grant_type=client_credentials&scope=all" https://api.mendeley.com/oauth/token | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["access_token"]')
token=$(curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -u 1739:84T68V4m9mOpsEAC -d "grant_type=client_credentials&scope=all" https://api.mendeley.com/oauth/token | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["access_token"]')
...
curl 'https://api.mendeley.com/documents' -X POST -H 'Authorization: Bearer '$token -H 'Content-Type: application/pdf' -H 'Content-Disposition: attachment; filename=$filename' --data-binary @$filename
curl 'https://api.mendeley.com/documents' -X POST -H 'Authorization: Bearer '$token -H 'Content-Type: application/pdf' -H 'Content-Disposition: attachment; filename='$filename --data-binary @$filename
...
-H 'Content-Disposition: attachment; filename="SSRN-id2021183.pdf"' \
--data-binary @SSRN-id2021183.pdf
